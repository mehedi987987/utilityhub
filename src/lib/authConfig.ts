...
export const GATE_MODE: GateMode = 'ai-only'

/**
 * Tools that hit a paid third-party API on every run.
 * face-beauty is deliberately NOT here: it runs entirely on canvas in the
 * browser and costs nothing, so gating it would only cost us search traffic.
 */
export const AI_TOOL_SLUGS = ['remove-bg', 'image-enhancer']
